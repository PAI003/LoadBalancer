...
#define NMAX 300001 // avem cel mult 99999 servere, fiecare cu cate 3 replici

struct load_balancer;
typedef struct load_balancer load_balancer;

struct load_balancer {
    linked_list_t *server_list; //lista de servere
    
    unsigned int hashring[NMAX];
    unsigned int hashring_size;     //declar hsshring ul ca array
    
    unsigned int stored;        //retin nr de obiecte stocate
};
