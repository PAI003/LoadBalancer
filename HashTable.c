				  ...
				  unsigned int value_size)
{
	...
	DIE(item == NULL, "could not allocate memory\n");

    item->key = malloc(key_size);
	DIE(item->key == NULL, "could not allocate memory\n");

    item->value = malloc(value_size);
	DIE(item->value == NULL, "could not allocate memory\n");
		...
		int (*compare_function)(void*, void*))
{
	...
	ht->buckets = malloc(table->hmax * sizeof(*table->buckets));
	DIE(ht->buckets == NULL, "could not allocate memory\n");
	...
	void *value, unsigned int value_size)
{
	...
	DIE(data_node == NULL, "could not allocate memory\n");

	data_node->key = malloc(key_size);
	DIE(data_node->key == NULL, "could not allocate memory\n");
	memcpy(data_node->key, key, key_size);

	data_node->value = malloc(value_size);
	DIE(data_node->value == NULL, "could not allocate memory\n");
